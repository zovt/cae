...
CharMapData graphics::fonts::get_char_map_data(std::string const& path, int px_sz) {
	...
	while (glyph_idx) {
		...
		result.char_to_metrics[chr].offset_x = metrics.horiBearingX;
		result.char_to_metrics[chr].offset_y = metrics.horiBearingY;
		result.char_to_metrics[chr].advance = metrics.horiAdvance;
	...
	uint32_t tex_height_px = n_glyphs * max_height;
	uint32_t tex_width_px = max_width;
	result.md.image_width = tex_width_px;
	result.md.image_height = tex_height_px;

	result.pixel_data.resize((size_t)(tex_height_px * tex_width_px));
	auto cursor = result.pixel_data.data();

	chr = FT_Get_First_Char(face, &glyph_idx);
	size_t i = 0;
	while (glyph_idx) {
		result.char_to_uv_locations[chr] = {0.f, ((float)i * (float)max_height) / result.md.image_height};
		...
		for (int row = 0; row < metrics.height; row++) {
			...
			cursor = cursor + max_width;
		}

		cursor = cursor + (max_height - metrics.height) * (max_width);
		i++;
		chr = FT_Get_Next_Char(face, chr, &glyph_idx);
	}

	auto space_idx = FT_Get_Char_Index(face, ' ');
